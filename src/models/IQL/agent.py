...
class IQL(nn.Module):
    def __init__(self,
        ...
        # Actor Network 
        self.actor_local = Actor(state_size[2], action_size, hidden_size).to(device)
        self.actor_optimizer = optim.Adam(self.actor_local.parameters(), lr=learning_rate)     
        
        # Critic Network (w/ Target Network)
        self.critic1 = Critic(state_size[2], action_size, hidden_size, 2).to(device)
        self.critic2 = Critic(state_size[2], action_size, hidden_size, 1).to(device)
        
        assert self.critic1.parameters() != self.critic2.parameters()
        
        self.critic1_target = Critic(state_size[2], action_size, hidden_size).to(device)
        self.critic1_target.load_state_dict(self.critic1.state_dict())

        self.critic2_target = Critic(state_size[2], action_size, hidden_size).to(device)
        self.critic2_target.load_state_dict(self.critic2.state_dict())

        self.critic1_optimizer = optim.Adam(self.critic1.parameters(), lr=learning_rate)
        self.critic2_optimizer = optim.Adam(self.critic2.parameters(), lr=learning_rate) 
        
        self.value_net = Value(in_channels=state_size[2], hidden_size=hidden_size).to(device)
    ...
    def get_action(self, state, eval=False):
        ...
        state = torch.from_numpy(state).unsqueeze(0).permute(0,3,1,2).float().to(self.device)
        with torch.no_grad():
